 ...
 */
contract HalalAssetRegistry is AccessControl, Pausable {
    // Role definitions
     ...
     */
    constructor(address _admin, address _shariahCommittee) {
    ...
    ) external onlyRole(SHARIAH_COMMITTEE_ROLE) whenNotPaused {
     ...
     */
    function addProhibitedIndustry(string calldata _industry) external onlyRole(SHARIAH_COMMITTEE_ROLE) {
     ...
     * @param _industry Name of the prohibited industry
     */
    function removeProhibitedIndustry(string calldata _industry) external onlyRole(SHARIAH_COMMITTEE_ROLE) {
     ...
     */
    function getCompliantAssetCount() external view returns (uint256) {
        return compliantAssets.length;
    }
