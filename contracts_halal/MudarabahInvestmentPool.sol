    ...
    Pausable 
{
    ...
    function invest(
        address token,
        uint256 amount
    ) external onlyRole(INVESTOR_ROLE) nonReentrant whenNotPaused notEmergencyShutdown onlyHalalAsset(token) {
    ...
    function withdraw(
        address token,
        uint256 amount
    ) external onlyRole(INVESTOR_ROLE) nonReentrant whenNotPaused onlyHalalAsset(token)  onlyOwner{
        require(amount > 0, "Amount must be greater than 0");
    ...
    function withdrawProfitShare(
        address token
    ) external onlyRole(INVESTOR_ROLE) nonReentrant whenNotPaused onlyHalalAsset(token) {
    ...
    function approveStrategy(
        bytes32 strategyId,
        uint256 allocation
    ) external onlyShariahAdvisor nonReentrant{
    ...
    ) external onlyRiskManager nonReentrant{
    ...
    ) external onlyMudarib nonReentrant{
        require(
     ...
     */
    function deactivateEmergencyShutdown() external onlyMudarib nonReentrant{
     ...
     */
    function pause() external onlyMudarib nonReentrant onlyOwner{
        _pause();
    }
     ...
     */
    function unpause() external onlyMudarib nonReentrant onlyOwner{
        _unpause();
    }
